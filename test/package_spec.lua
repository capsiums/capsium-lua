describe("Capsium Package model", function()
  ...
  describe("dataset loading", function()
    ...
    it("rejects SQLite datasets with not_implemented kind (issue #10)", function()
      local data, err, kind = package:get_dataset("sqlite_ds")
      assert.is_nil(data)
      assert.is_truthy(tostring(err):find("SQLite"))
      assert.equals("not_implemented", kind)
    end)

    it("rejects unknown datasets with unknown kind (issue #10)", function()
      local data, err, kind = package:get_dataset("nope")
      assert.is_nil(data)
