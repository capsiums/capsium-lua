...
def test_package_css(api_client):
    """Test that CSS files in Capsium packages are served correctly."""
    response = api_client.get("/capsium/test-package-0.1.0/styles.css")
    assert response.status_code == 200

    # Verify the content type is CSS
    assert "text/css" in response.headers.get("Content-Type", "")

    # Verify the content contains expected CSS
    assert "Capsium Test Package Styles" in response.text

def test_package_js(api_client):
    """Test that JavaScript files in Capsium packages are served correctly."""
    response = api_client.get("/capsium/test-package-0.1.0/script.js")
    assert response.status_code == 200

    # Verify the content type is JavaScript
    assert "javascript" in response.headers.get("Content-Type", "").lower()

    # Verify the content contains expected JavaScript
    assert "Capsium Test Package JavaScript" in response.text
...
def test_nonexistent_file_in_package(api_client):
    ...
    response = api_client.get("/capsium/test-package-0.1.0/nonexistent.html")
    assert response.status_code == 404

def test_package_index_routes(api_client):
    """Test that different index routes work for packages."""
    # Test root path
    response1 = api_client.get("/capsium/test-package-0.1.0/")
    assert response1.status_code == 200

    # Test /index path
    response2 = api_client.get("/capsium/test-package-0.1.0/index")
    assert response2.status_code == 200

    # Test /index.html path
    response3 = api_client.get("/capsium/test-package-0.1.0/index.html")
